...
class Fuzzer
  ...
  TARGET_PORT = 9292 # Default rackup port
  USE_SSL = false   # Rack::Handler::WEBrick by default is HTTP
  ITERATIONS = 20
  ...
  def self.run(iterations = ITERATIONS)
    iterations.times do |i|
      method = HTTP_METHODS.sample
      path = FUZZ_PATHS.sample
      begin
        uri = URI::HTTP.build(host: TARGET_HOST, port: TARGET_PORT, path: path)
